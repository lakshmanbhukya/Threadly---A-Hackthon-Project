...
router.post("/login", validateLogin, async (req, res) => {
  try {
    ...
    if (!logintoken || !password) {
      return res
        .status(400)
        .json({ error: "Login token and password required" });
    }

    const user = await User.findOne({
      $or: [{ email: logintoken }, { username: logintoken }],
    });
    ...
    if (!user.isActive) {
      return res
        .status(403)
        .json({ error: "Account is inactive or scheduled for deletion" });
    }
...
router.delete("/account", async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Account deletion requested. Account will be deleted in 30 days.",
...
router.post("/account/cancel-deletion", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ error: "No deletion request found for this account" });
    }
