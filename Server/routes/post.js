  ...
  async (req, res) => {
    try {
      ...
      const post = new Post({
        ...
        createdBy:
          isAnonymous === "true" || !req.session?.userId
            ? null
            : req.session.userId,
...
router.post("/:id/downvote", async (req, res) => {
  try {
    ...
    await post.save();
    res.json({
      message: isDownvoted ? "Post undownvoted" : "Post downvoted",
...
// GET /posts/user/:userId - Get posts/comments created by a user
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch user's posts/comments" });
  }
});
